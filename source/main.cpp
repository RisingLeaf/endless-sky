		...
		const string &testToRunName, bool debugMode)
{
			...
			&isFastForward]
	{
		...
		for(const auto &event : GameWindow::FetchEvents())
		{
			...
			else if(activeUI.Handle(event))
			{
				// The UI handled the event.
					...
					&& !Command(GLFW_KEY_CAPS_LOCK).Has(Command::FASTFORWARD))
			{
		...
		// on keypress.
		if(Command(GLFW_KEY_CAPS_LOCK).Has(Command::FASTFORWARD))
			isFastForward = GameWindow::ModActive(GameWindow::MOD_CAPS);
	};
	...
	if(!testContext.CurrentTest())
	{
		while(!menuPanels.IsDone())
		{
			...
			if(GameWindow::ModActive(GameWindow::MOD_CAPS) && inFlight && debugMode)
			{
				if(frameRate > 10)
				{
