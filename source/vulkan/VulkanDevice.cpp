...
void VulkanDevice::CreateInstance()
{
	...
	appInfo.pApplicationName = "Endless Sky";
	appInfo.applicationVersion = VK_MAKE_VERSION(1, 0, 0);
	appInfo.pEngineName = "ES Engine";
...
void VulkanDevice::PickPhysicalDevice()
{
	...
	Logger::LogError("Device count: " + std::to_string(deviceCount));
	...
	Logger::LogError("Physical device: " + std::string(properties.deviceName));
}
...
{
	GameWindow::CreateWindowSurface(instance, &surface_);
}
...
void VulkanDevice::HasRequiredInstanceExtensions()
{
	...
	Logger::LogError("Available extensions:");
	std::unordered_set<std::string> available;
	for(const auto &extension : extensions)
	{
		Logger::LogError("\t" + std::string(extension.extensionName));
		available.insert(extension.extensionName);
	}

	Logger::LogError("Required extensions:");
	auto requiredExtensions = GetRequiredExtensions();
	for(const auto &required : requiredExtensions)
	{
		Logger::LogError("\t" + std::string(required));
